...
def runApplication():
	...
	if selectedTask == "parsing":
		...
		for sid, sentence in enumerate(lboxContent):
			...
			nodes = list(plotTree.dependencyGraph(poutput))
		...
		for sent_id in lbox.nlpprocesses['parsing']:
			tags = [(node.form, node.ctag) for node in lbox.nlpprocesses['parsing'][sent_id][1]]
			lbox.nlpprocesses['tagging'][sent_id] = tags
		lbox.nlpprocesses['stash'] = True
	elif selectedTask == "ontorels":
		lbox.nlpprocesses['stash'] = True
		if lbox.nlpprocesses['ontorels']:return
		houtput = nlpTools.run_onto(lboxContent)
		return
		poutput = houtput.split("\n")[2:-2]
		lbox.nlpprocesses['ontorels'] = poutput
		lbox.nlpprocesses['stash'] = True
	...
	for nlproc, output in lbox.nlpprocesses.items():
		...
		if nlproc == "tagging":
			...
			with open("%s.onto"%base, "w") as ofp:
				for oid, opair in enumerate(output,1):
					first, second, relation, confidence, similarity = opair.strip().split("\t")
					ofp.write("%s\t%s\t%s\t%s\t%s\t%s\n" % (oid, first, second, relation, confidence, similarity))
...
def askPopup():
	...
	what = askyesno(message="Would you like to save the system outputs?", title="Save the outputs?")
	return what
...
if __name__ == "__main__":
    ...
    img = PhotoImage(file='%s/clear.png' % current_path)
