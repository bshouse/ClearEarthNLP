...
class OntoWidget(object):
    def __init__(self, annotations=dict(),content=dict()):
        ...
        #self.tree.pack()
        self.style.configure('Treeview', rowheight=self.itemfont.metrics()['linespace'])
    ...
    def contentReader(self):
        ...
        for idx, relation_info in enumerate(self.annotations['ontorels']):
            phrase1, phrase2, similarity, confidence, relation = relation_info
    ...
    def saveAnnotations(self):
        ...
        for key, value in sorted(self.content.items(), key=lambda x: x[1][0]):
            ...
            self.annotations['ontorels'][value[0]][-1] = value[-1]
    ...
    def askPopup(self):
        what = askokcancel(message="Would you like to save the modified system outputs?", title="Save the changes?")
        return what
